...
public class BugRepository : TodoRepository<Bug>
{
    ...
    public async override Task AddAsync(Bug bug)
    {
        var existingBug = await Context.Bugs.FirstOrDefaultAsync(b => b.Id == bug.Id);
        
        var user = await Context.Users.FirstOrDefaultAsync(user => user.Id == bug.CreatedBy.Id);
    ...
    public async override Task<Bug> GetAsync(Guid id)
    {
