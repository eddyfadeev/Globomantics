...
using User = Globomantics.Infrastructure.Data.Models.User;
...
public class FeatureRepository : TodoRepository<Feature>
{
    ...
    public async override Task AddAsync(Feature feature)
    {
        var existingFeature = await Context.Features.FirstOrDefaultAsync(f => f.Id == feature.Id);
    ...
    private async Task UpdateAsync(Feature feature, Data.Models.Feature featureToUpdate, Models.User user)
    {
    ...
    public async override Task<Feature> GetAsync(Guid id)
    {
        var data = await Context.Features.SingleAsync(f => f.Id == id);
